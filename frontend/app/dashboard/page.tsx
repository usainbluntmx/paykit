...
export default function Home() {
  ...
  async function handleRegisterAgent() {
    ...
    try {
      ...
      const tx = await program.methods
        .registerAgent(agentName, new BN(limitLamports), bps)
    ...
    try {
      ...
      const valid = all.filter((a: any) => {
        try {
          ...
          if (a.account.dailyLimitBps === undefined || a.account.dailyLimitBps === null) return false;
          return true;
        } catch { return false; }
      });
      setAgents(valid.map((a: any) => ({
        ...
        dailyLimitBps: a.account.dailyLimitBps || 1000,
      })));
  ...
  async function handleAudit() {
    ...
    try {
      ...
      setAuditResult({
        ...
        dailyLimitBps: agent.dailyLimitBps || 1000,
      });
    } catch { setAuditError("AGENT NOT FOUND OR INVALID PDA"); }
    setAuditLoading(false);
  }

  async function handleRenewAgent(agentName: string) {
    if (!program || !wallet.publicKey) return;
    setStatus(`RENEWING ${agentName.toUpperCase()}...`);
    setStatusType("loading");
    try {
      const agentPDA = getAgentPDA(wallet.publicKey, agentName);
      const tx = await program.methods
        .renewAgent(new BN(31_536_000))
        .accounts({ agent: agentPDA, owner: wallet.publicKey })
        .rpc({ skipPreflight: true, commitment: "confirmed" });
      setLastTx(tx);
      setStatus(`AGENT "${agentName.toUpperCase()}" RENEWED — +365 DAYS`);
      setStatusType("ok");
      showToast(`${agentName} renewed for 365 days`);
      await fetchAgents(program);
    } catch (e: any) {
      setStatus(`ERR: ${e.message.slice(0, 60)}`);
      setStatusType("error");
  ...
  async function handleDeactivateAgent(agentName: string) {
    if (!program || !wallet.publicKey) return;
    setStatus(`DEACTIVATING ${agentName.toUpperCase()}...`);
    setStatusType("loading");
    try {
      const agentPDA = getAgentPDA(wallet.publicKey, agentName);
      const tx = await program.methods
        ...
        .rpc({ skipPreflight: true, commitment: "confirmed" });
      setLastTx(tx);
      setStatus(`AGENT "${agentName.toUpperCase()}" DEACTIVATED`);
      setStatusType("ok");
      showToast(`${agentName} deactivated`, "error");
      await fetchAgents(program);
    } catch (e: any) {
      setStatus(`ERR: ${e.message.slice(0, 60)}`);
      setStatusType("error");
  ...
  async function handleReactivateAgent(agentName: string) {
    if (!program || !wallet.publicKey) return;
    setStatus(`REACTIVATING ${agentName.toUpperCase()}...`);
    setStatusType("loading");
    try {
      const agentPDA = getAgentPDA(wallet.publicKey, agentName);
      const tx = await program.methods
        ...
        .rpc({ skipPreflight: true, commitment: "confirmed" });
      setLastTx(tx);
      setStatus(`AGENT "${agentName.toUpperCase()}" REACTIVATED`);
      setStatusType("ok");
      showToast(`${agentName} reactivated`);
      await fetchAgents(program);
    } catch (e: any) {
      setStatus(`ERR: ${e.message.slice(0, 60)}`);
      setStatusType("error");
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "10px", padding: "8px 14px", background: "rgba(0,255,136,0.02)", border: "1px solid rgba(0,255,136,0.08)", borderLeft: `3px solid ${statusColor}`, borderRadius: "2px", marginBottom: "16px" }}>
        ...
        {lastTx && (
          <button
            ...
            style={{ background: "transparent", border: "1px solid rgba(0,255,136,0.2)", color: "#9aeab0", fontFamily: "'Share Tech Mono', monospace", fontSize: "11px", padding: "4px 10px", borderRadius: "2px", cursor: "pointer", letterSpacing: "0.1em", whiteSpace: "nowrap", transition: "all 0.2s", flexShrink: 0 }}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      {agents.slice(agentPage * AGENTS_PER_PAGE, (agentPage + 1) * AGENTS_PER_PAGE).map(a => (
                        <div key={a.pda} style={{ padding: "12px", background: "rgba(0,255,136,0.02)", border: "1px solid rgba(0,255,136,0.08)", borderRadius: "3px", fontSize: "13px" }}>
                          ...
                          <div style={{ display: "flex", gap: "8px", color: "#9aeab0", fontSize: "11px", flexWrap: "wrap" }}>
                          ...
                          <div style={{ display: "flex", gap: "8px", marginTop: "8px", flexWrap: "wrap" }}>
                            {a.expiresAt > 0 && (
                              <button onClick={() => handleRenewAgent(a.name)} style={{ background: "transparent", border: "1px solid rgba(0,255,136,0.15)", color: "#6aaa80", fontFamily: "'Share Tech Mono', monospace", fontSize: "10px", padding: "3px 10px", borderRadius: "2px", cursor: "pointer", letterSpacing: "0.1em", transition: "all 0.2s" }}
                            ...
                            {a.isActive && a.expiresAt > 0 && (
                              <button onClick={() => handleDeactivateAgent(a.name)} style={{ background: "transparent", border: "1px solid rgba(255,60,90,0.15)", color: "#6a3a3a", fontFamily: "'Share Tech Mono', monospace", fontSize: "10px", padding: "3px 10px", borderRadius: "2px", cursor: "pointer", letterSpacing: "0.1em", transition: "all 0.2s" }}
                            ...
                            {!a.isActive && a.expiresAt > 0 && (
                              <button onClick={() => handleReactivateAgent(a.name)} style={{ background: "transparent", border: "1px solid rgba(255,184,0,0.15)", color: "#6a6a3a", fontFamily: "'Share Tech Mono', monospace", fontSize: "10px", padding: "3px 10px", borderRadius: "2px", cursor: "pointer", letterSpacing: "0.1em", transition: "all 0.2s" }}
              ...
              {auditResult && (
                <div style={{ marginTop: "12px", padding: "14px", background: "rgba(0,255,136,0.02)", border: "1px solid rgba(0,255,136,0.1)", borderRadius: "3px", fontSize: "13px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", color: "#9aeab0", fontSize: "13px" }}>
                    ...
                    <div>EXPIRES<div style={{ color: auditResult.expiresAt > 0 ? (auditResult.expiresAt * 1000 > Date.now() ? "#6aaa80" : "#ff3c5a") : "#3a6a4a", marginTop: "3px" }}>
                      {auditResult.expiresAt > 0 ? new Date(auditResult.expiresAt * 1000).toLocaleDateString() : "LEGACY"}
                    </div></div>
                    <div>STATUS<div style={{ color: auditResult.isActive ? "#00ff88" : "#ff3c5a", marginTop: "3px" }}>
                      {auditResult.isActive ? "ACTIVE" : "INACTIVE"}
                    </div></div>
                  </div>
                </div>
              )}
